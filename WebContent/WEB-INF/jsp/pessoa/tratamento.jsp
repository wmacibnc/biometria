		...
		<div class="panel panel-default">
           ...
           <span>Tratamentos</span>
			...
			<c:if test="${mensagem != null}">
				...
				<label>Situação do Tratamento: </label>
				<select name="situacao" class="form-control" aria-describedby="basic-addon1" style="margin-left: 5px;" id="idSituacao">
					<option value="1" ${situacao eq 1 ? 'selected' : ''}> Em aberto </option>
					<option value="2" ${situacao eq 2 ? 'selected' : ''}> Finalizados </option>
					<option value="3" ${situacao eq 3 ? 'selected' : ''}> Todos </option>
				</select>
				
				<script type="text/javascript">
					$(document).ready(function(){
						$('#idSituacao').on('change', function(){
							var self = $(this);
							var selecionado = self.val();
							var url = "<c:url value='/tratamentos/${paciente.id}/" + selecionado + "'/>";
