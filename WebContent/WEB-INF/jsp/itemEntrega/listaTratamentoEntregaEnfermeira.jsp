  ...
  right: 0%;
	...
	<div class="container">
		<div class="row">
			...
			<p>
				<b>Enfermeira:</b> ${enfermeira.nome}
			</p>
			
		<form action="<c:url value="/finalizarEntregaMedicamento"/>" method="POST">
		
			<table class="table">
				<thead>
					<tr>
					<th class="sortable">Sessão</th>
						<th class="sortable">Médico</th>
					<th class="sortable">Tratamento</th>
						<th class="sortable">Medicamento</th>
						<th class="sortable">Quantidade Total</th>
						<th class="sortable">Entrega</th>
					</tr>
				</thead>
				...
				<tbody>
					<c:forEach var="tratamento" items="${tratamentoList}" varStatus="cont">
					  ...
					  <input type="hidden" name="itemEntregas[${tratamento.id}].quantidadeEntrege" value="1" />
					  ...
					  <c:if test="${tratamento.totalEntregue < tratamento.quantidadeTotal}">
						<tr class="odd">
							...
							<td>
								<label class="on-off">
    								<input type="checkbox" name="itemEntregas[${tratamento.id}].sim"/>
    								<span><a></a></span>
  								</label>
							</td>
						</tr>
...
<div id="footer">
	<a href="<c:url value="/telaInicial"/>">
		...
		<button type="submit" class="btn btn-success btn-footer" name="_method" value="POST" onclick="return confirm('Confirma a entregar dos medicamentos?');">Entregar</button>
</div>
